...
module.exports.run = async (client, message, args, fs, colors, database, dataServer, language) => {
    ...
    if(!args[0]){ return await msg.sendMsg("INVALID_ARGS_TEXTS",message,dataServer)}

    switch (args[0]) {
        case "joinText":
            if(!args[1]){ return await msg.sendMsgA(language("MISSED_TEXT",dataServer.prefix,"jq"),message,dataServer) }
        ...
        case "quitText":
            ...
            break;
        case "lvlUp":
            if(!args[1]){ return await msg.sendMsgA(language("MISSED_TEXT",dataServer.prefix,"lvlup"),message,dataServer) }
